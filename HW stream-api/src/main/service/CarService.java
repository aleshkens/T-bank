...
public class CarService {
    ...
    public List<String> getConditions(List<Car> cars) {
        return cars.stream().map(Car::getCondition).map(Condition::getText).collect(Collectors.toList());
    }
    ...
    public List<Car> getNewCars(List<Car> cars) {
        return cars.stream().filter(car -> car.getCondition().getText().equals("NEW")).collect(Collectors.toList());
    }
    ...
    public List<Car> getTwoBrokenCar(List<Car> cars) {
        return cars.stream().filter(car -> car.getCondition().getText().equals("BROKEN")).limit(2).collect(Collectors.toList());
    }
    ...
    public Boolean checkBrokenCarsAge(List<Car> cars) {
        return cars.stream().filter(car -> car.getCondition().getText().equals("BROKEN")).allMatch(car -> car.getAge() > 10);
    }
    ...
    public Boolean checkCarOwnerName(List<Car> cars) {
        return cars.stream().filter(car -> car.getCondition().getText().equals("USED")).flatMap(car -> car.getOwners().stream()).anyMatch(owner -> owner.getName().equals("Adam"));
    }
